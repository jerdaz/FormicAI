...
//            scoutInfo.sources[i++] = {pos_x: source.pos.x, pos_y: source.pos.y }
    ...
    scoutInfo.hasEnemyCreeps = this.find(FIND_HOSTILE_CREEPS).length > 0;
}
...
Room.prototype.findHarvestRooms = function() {
    ...
    //uit cache halen indien mogelijk (tijdelijk uitgeschakeld tot ik verbeterde scouting heb)
            ...
            if (dist == depth) {
                ...
                for (var exitkey in exits) {
                    ...
                    if ((curRoomDist == undefined || curRoomDist > depth + 1) )  {
                        ...
                        if (isValidHarvestRoom(roomName, this.controller.level)) {
                            // resultaat opslaan
...
Room.prototype.getStoredEnergy = function() {
    let result = 0;
    if (this.storage) result += this.storage.store.energy;
    if (this.terminal) result += this.terminal.store.energy;
    else {
        for (let container of this.getContainers()) result += container.store.energy;
    }
    return result;
}
...
Room.prototype.findEnergyDropPoints = function(amount: number) {
    if (this.storage && this.storage.store.energy < MIN_STORAGE_ENERGY) return [this.storage];
